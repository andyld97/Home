    ...
    public static class Serialization
    {
        ...
        public static T ReadString<T>(string xmlData, System.Text.Encoding encoding)
        {
            if (!string.IsNullOrEmpty(xmlData) && encoding != null)
                return Serialization.ReadBytes<T>(encoding.GetBytes(xmlData));
            else
                return default(T);
        }
        ...
        /// <returns></returns>
        public static object ReadBytes(byte[] xmlBytes, Type type)
        {
        ...
        /// <returns></returns>
        public static object ReadString(string xmlData, Type type, System.Text.Encoding encoding)
        {
            if (string.IsNullOrEmpty(xmlData))
                return null;

            return Serialization.ReadBytes(encoding.GetBytes(xmlData), type);
        }
        ...
        /// <returns></returns>
        public static T Read<T>(string fileName, Mode mode = Mode.Normal)
        {
            try
            {
                if (mode == Mode.Normal)
        ...
        {
            byte[] result = Serialization.SaveToBytes<T>(instance, additionalTypes);
            if (result != null)
        ...
        public static string SaveToString(object instance, Type type, System.Text.Encoding encoding, Type[] additionalTypes = null)
        {
            if (instance != null && type != null && encoding != null)
                return encoding.GetString(Serialization.SaveToBytes(instance, type, additionalTypes));
